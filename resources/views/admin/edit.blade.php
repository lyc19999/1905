...
<body>
    <form action="{{url('admin/update',$data->admin_id)}}" method="post" enctype="multipart/form-data">
        ...
        @csrf
        用户名称: <input type="text" name="admin_name" id="name" value="{{$data->admin_name}}"><span style=color:red;>{{$errors->first('admin_name')}}</span><br>
        ...
        手机号: <input type="tel" name="admin_tel" id="tel" value="{{$data->admin_tel}}"><span style=color:red;>{{$errors->first('admin_tel')}}</span><br>
        <img src="{{env('UPLOAD_URL')}}{{$data->admin_head}}" width="50px" height="50px">
        头像: <input type="file" name="admin_head"><span style=color:red;>{{$errors->first('admin_head')}}</span><br>
        性别:
        @if($data->admin_sex==1)
        <input type="radio" name="admin_sex" value="1" checked>男
        <input type="radio" name="admin_sex" value="2">女<br>
        @else
        <input type="radio" name="admin_sex" value="1">男
        <input type="radio" name="admin_sex" value="2" checked>女<br>
        @endif
        邮箱: <input type="text" name="admin_email" id="email" value="{{$data->admin_email}}"><span style=color:red;>{{$errors->first('admin_email')}}</span><br>
